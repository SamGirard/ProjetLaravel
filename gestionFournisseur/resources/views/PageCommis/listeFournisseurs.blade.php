...
<div class="flex min-h-screen">
        ...
        <div class="mt-1 pb-4 flex justify-between items-center">
            ...
            <div class="flex items-center space-x-2">
                <button id="open-list" type="button"
                    class="text-white bg-blue-500 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
    ...
    filteredFournisseur.value = JSON.stringify([]); // Add your logic for filteredFournisseurs if necessary
    checkedFournisseur.value = JSON.stringify(checkedFournisseurs); // Assuming checkedFournisseurs is an object or array
}

document.getElementById('exportForm').onsubmit = function (event) {
    // Collect checked fournisseurs
    let ids = [];
    Object.keys(checkedFournisseurs).forEach(id => {
        if (checkedFournisseurs[id]) {
            ids.push(id);
    ...
    if (ids.length > 0) {
        populateInputs(); // Only populate inputs if valid
